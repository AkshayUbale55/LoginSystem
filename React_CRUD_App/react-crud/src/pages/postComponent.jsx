...
export const PostComponent = (props) => {
    ...
    const likeRef = collection(db, "post-likes")
    ...
    const addLike = async () => {
        try {
           await addDoc(likeRef, { userId: user?.uid, postId: post.id })
            if (user) setLikeCount((prev) => prev ? [...prev, { userId: user.uid }] : [{ userId: user.uid }])
        }
        catch (err) {
            console.log(err)
        }
    ...
    const removeLike = async () => {
        ...
        try {
            ...
            const deleteLikeDataQuery = query(likeRef, where("postId", "==", post.id) && where("userId", "==", user?.uid))
            const unLiketoDelete = await getDocs(deleteLikeDataQuery)
            // const likeId = unLiketoDelete.docs[0].id
            const unlikePost = doc(db, "post-likes",unLiketoDelete.docs[0].id)
            await deleteDoc(unlikePost)
            if (user) setLikeCount((prev) => prev && prev.filter((like)=> like.userId !== user.uid))
        }
        catch (e) {
            console.log(e)
        }
    ...
    const hasUserLiked = likeCount?.find((likes) => likes.userId === user?.uid)
    ...
    // const truncatedDescription = post.description.slice(0,30)
            ...
            <div className="relative p-4 w-auto h-auto">
                <div className="px-8 py-4 pt-24 bg-white rounded-lg shadow dark:bg-gray-800">
                    ...
                    <div className="flex items-center justify-between w-40 pt-8 mx-auto text-gray-500 border-t border-gray-200">
                        ...
                        <button onClick={hasUserLiked ? removeLike : addLike } className="text-2xl hover:animate-bounce">{hasUserLiked ? <>&#x1F44E;</> : <>&#x1F44D;</>}</button>
